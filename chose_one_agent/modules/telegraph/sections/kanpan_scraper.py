...
class KanpanScraper(BaseTelegraphScraper):
    ...
    def scrape_section(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                if "kanpan" not in current_url.lower() and "看盘" not in current_content:
    ...
    def run(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if self.page:
                # 导航到电报页面
                logger.info("导航到电报看盘页面")
                try:
                    # 首先尝试直接导航到看盘板块
                    telegraph_kanpan_url = f"{self.base_url}/telegraph/kanpan"
                    self.page.goto(telegraph_kanpan_url)
                    self.page.wait_for_load_state("networkidle")
                    time.sleep(2)
                    logger.info(f"直接导航到电报看盘页面URL: {telegraph_kanpan_url}")
                except Exception as e:
                    logger.error(f"直接导航到电报看盘页面失败: {e}")
                    # 如果直接导航失败，尝试从电报主页导航
                    try:
                        ...
                        logger.error(f"从电报主页导航到看盘页面失败: {e}")
                        return []
